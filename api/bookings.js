  ...
  try {
    ...
    if (req.method === "POST") {
      ...
      if (!name || !start || !end) {
        return res.status(400).json({
          error: "Namn, incheckning och utcheckning behövs."
        });
      }
      ...
      if (nights < 1) {
        return res.status(400).json({
          error: "Utcheckning måste vara efter incheckning."
        });
      }

      if (nights > 3) {
        return res.status(400).json({
          error: "Max antal nätter per bokning är 3."
        });
      }
      ...
      if (hasConflict) {
        return res.status(400).json({
          error: "Datumen krockar med en befintlig bokning eller ligger direkt intill en annan."
        });
      }
    ...
    if (req.method === "DELETE") {
      ...
      if (!deleteResponse.ok) {
        throw new Error(deleteData.error?.message || "Kunde inte ta bort bokningen.");
      }
